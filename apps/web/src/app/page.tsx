  ...
  const fetchBookings = async () => {
    try {
      ...
      const formattedBookings = response.data.map((b: any) => {
        ...
        return {
          id: b.id.toString(),
          title: `${b.title} (${b.status === 'PENDING' ? 'Pendente' : b.room.name})`,
          start: b.start_time,
          end: b.end_time,
        ...
        <div className="flex-1 p-6 md:p-10 overflow-y-auto">
          ...
          {(activeTab === 'approvals' && currentUser?.role === 'ADMIN') && (
            <div className="h-full animation-fade-in">
              <header className="mb-8"><h2 className="text-3xl font-bold mb-2">Aprovações Pendentes</h2><p className="text-zinc-400">Analise e aprove as solicitações de reserva.</p></header>
              <div className="grid grid-cols-1 gap-4">
                {bookings.filter(b => b.extendedProps.status === 'PENDING').length === 0 && (
                  <div className="glass-panel p-10 rounded-2xl border border-white/5 text-center text-zinc-500">Nenhuma aprovação pendente.</div>
                )}
                {bookings.filter(b => b.extendedProps.status === 'PENDING').map(b => (
                  <div key={b.id} className="glass-panel p-6 rounded-2xl border border-white/5 flex items-center justify-between group hover:border-yellow-500/30 transition-all">
                    <div>
                      <h3 className="text-lg font-bold text-white">{b.extendedProps.title}</h3>
                      <div className="flex gap-4 mt-1 text-sm text-zinc-400">
                        <p><strong className="text-zinc-300">Sala:</strong> {b.extendedProps.room.name}</p>
                        <p><strong className="text-zinc-300">Solicitante:</strong> {b.extendedProps.user.name}</p>
                        <p><strong className="text-zinc-300">Horário:</strong> {new Date(b.start).toLocaleString('pt-br')}</p>
                      </div>
                    </div>
                    <div className="flex gap-3">
                      <button onClick={() => handleUpdateBookingStatus(b.id, 'APPROVED')} className="p-2 bg-green-500/10 text-green-500 rounded-lg hover:bg-green-500/20 border border-green-500/20 transition-all" title="Aprovar"><CheckCircle size={20} /></button>
                      <button onClick={() => handleUpdateBookingStatus(b.id, 'REJECTED')} className="p-2 bg-red-500/10 text-red-500 rounded-lg hover:bg-red-500/20 border border-red-500/20 transition-all" title="Rejeitar"><Ban size={20} /></button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {(activeTab === 'rooms' && currentUser?.role === 'ADMIN') && (
            <div className="h-full animation-fade-in">
              <header className="mb-8 flex justify-between items-center">
                ...
                <button onClick={() => { setEditingRoom(null); setNewRoom({ name: '', cap: '', res: '' }); setIsRoomModalOpen(true); }} className="flex items-center gap-2 bg-primary hover:bg-primary/80 text-primary-foreground px-6 py-2.5 rounded-lg font-medium shadow-[0_0_15px_rgba(59,130,246,0.4)]"><Plus size={18} /> Nova Sala</button>
              </header>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {rooms.map((room) => (
                  <div key={room.id} className="glass-panel p-6 rounded-2xl border border-white/5 relative group hover:border-primary/30 flex flex-col justify-between min-h-[160px]">
                    <div><div className={`absolute top-6 right-6 w-3 h-3 rounded-full shadow-[0_0_8px_currentColor] ${room.is_active ? 'bg-green-500 text-green-500' : 'bg-zinc-600 text-zinc-600'}`} /><h3 className="text-xl font-bold mb-2">{room.name}</h3><div className="space-y-1 text-zinc-400 text-sm"><p><strong className="text-zinc-300">Capacidade:</strong> {room.capacity} pessoas</p><p><strong className="text-zinc-300">Recursos:</strong> {room.description}</p></div></div>
                    <div className="mt-6 pt-4 border-t border-white/5 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity"><button onClick={() => handleEditRoom(room)} className="flex-1 flex items-center justify-center gap-2 py-2 bg-blue-500/10 text-blue-400 rounded-lg hover:bg-blue-500/20 border border-blue-500/20 text-xs font-bold"><Edit2 size={14} /> Editar</button><button onClick={() => handleDeleteRoom(room.id)} className="flex-1 flex items-center justify-center gap-2 py-2 bg-red-500/10 text-red-400 rounded-lg hover:bg-red-500/20 border border-red-500/20 text-xs font-bold"><Trash2 size={14} /> Excluir</button></div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {(activeTab === 'users' && currentUser?.role === 'ADMIN') && (
            <div className="h-full animation-fade-in">
               <header className="mb-8 flex justify-between items-center">
                ...
                <button onClick={() => { setEditingUser(null); setNewUser({ name: '', username: '', role: 'USER', password: '' }); setIsUserModalOpen(true); }} className="flex items-center gap-2 bg-primary hover:bg-primary/80 text-primary-foreground px-6 py-2.5 rounded-lg font-medium shadow-[0_0_15px_rgba(59,130,246,0.4)]"><Plus size={18} /> Novo Usuário</button>
              </header>
              <div className="glass-panel rounded-2xl overflow-hidden border border-white/5">
                <table className="w-full text-left">
                  <thead className="bg-white/5 border-b border-white/5"><tr><th className="p-4 font-medium text-zinc-400">Nome</th><th className="p-4 font-medium text-zinc-400">Usuário</th><th className="p-4 font-medium text-zinc-400">Papel</th><th className="p-4 font-medium text-zinc-400">Ações</th></tr></thead>
                  <tbody>{users.map(user => (<tr key={user.id} className="border-b border-white/5 group hover:bg-white/5 transition-colors"><td className="p-4">{user.name}</td><td className="p-4">{user.username}</td><td className="p-4"><span className={`px-2 py-1 rounded text-xs font-bold border ${user.role === 'ADMIN' ? 'bg-primary/20 text-primary border-primary/30' : 'bg-zinc-800 text-zinc-300 border-zinc-700'}`}>{user.role}</span></td><td className="p-4"><div className="flex gap-2"><button className="p-2 text-blue-400 hover:bg-blue-400/10 rounded-lg transition-colors" onClick={() => handleEditUser(user)} title="Editar"><Edit2 size={18} /></button><button className="p-2 text-red-400 hover:bg-red-400/10 rounded-lg transition-colors" onClick={() => handleDeleteUser(user.id)} title="Excluir"><Trash2 size={18} /></button></div></td></tr>))}</tbody>
                </table>
              </div>
            </div>
          )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animation-fade-in">
          <div className="glass-panel w-full max-w-md p-6 rounded-2xl border border-white/10 shadow-2xl relative">
            ...
            <div className="flex items-center gap-3 mb-6">
              <div className="p-2 bg-primary/20 text-primary rounded-lg border border-primary/30"><Clock size={20} /></div>
              <h3 className="text-xl font-bold">Solicitar Reserva</h3>
            </div>
