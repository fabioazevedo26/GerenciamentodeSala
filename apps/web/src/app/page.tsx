...
export default function Dashboard() {
  ...
  const handleDeleteRoom = async (id: number) => {
    if (confirm('Tem certeza que deseja excluir esta sala? Todas as reservas vinculadas podem ser afetadas.')) {
      try {
      ...
      <div className="flex min-h-screen items-center justify-center p-4 relative overflow-hidden bg-[#0A0A0A]">
        ...
        <div className="glass-panel w-full max-w-md p-8 rounded-2xl relative z-10 animation-fade-in border border-white/10 shadow-2xl">
          <div className="flex flex-col items-center mb-8">
            <div className="w-16 h-16 rounded-2xl bg-primary/20 text-primary flex items-center justify-center border border-primary/30 mb-4">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <input type="text" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-primary/50" placeholder="admin" value={username} onChange={(e) => setUsername(e.target.value)} required />
            </div>
            <div>
              <label className="block text-sm font-medium text-zinc-300 mb-1">Senha</label>
              <input type="password" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-primary/50" placeholder="admin" value={password} onChange={(e) => setPassword(e.target.value)} required />
            </div>
            <button type="submit" className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-medium rounded-lg px-4 py-3 mt-4 transition-all shadow-[0_0_15px_rgba(59,130,246,0.4)]">
              Entrar no Sistema
            </button>
          </form>
        </div>
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
        <header className="md:hidden glass-panel border-b border-white/5 p-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-primary/20 text-primary flex items-center justify-center border border-primary/30"><CalendarIcon size={16} /></div>
            <h1 className="font-bold">SMGR</h1>
          </div>
          <button onClick={() => setIsAuthenticated(false)} className="text-zinc-400 hover:text-red-400"><LogOut size={20} /></button>
        </header>
          ...
          {(activeTab === 'rooms' && currentUser?.role === 'ADMIN') && (
            <div className="h-full animation-fade-in">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {rooms.map((room) => (
                  <div key={room.id} className="glass-panel p-6 rounded-2xl border border-white/5 relative group hover:border-primary/30 transition-all flex flex-col justify-between min-h-[160px]">
                    <div>
                      <div className={`absolute top-6 right-6 w-3 h-3 rounded-full shadow-[0_0_8px_currentColor] ${room.is_active ? 'bg-green-500 text-green-500' : 'bg-zinc-600 text-zinc-600'}`} />
                      <h3 className="text-xl font-bold mb-2">{room.name}</h3>
                      <div className="space-y-1 text-zinc-400 text-sm">
                        <p><strong className="text-zinc-300">Capacidade:</strong> {room.capacity} pessoas</p>
                        <p><strong className="text-zinc-300">Recursos:</strong> {room.description}</p>
                      </div>
                    </div>
                    <div className="mt-6 pt-4 border-t border-white/5 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => handleEditRoom(room)} className="flex-1 flex items-center justify-center gap-2 py-2 bg-blue-500/10 text-blue-400 rounded-lg hover:bg-blue-500/20 border border-blue-500/20 transition-all text-xs font-bold">
                        <Edit2 size={14} /> Editar
                      </button>
                      <button onClick={() => handleDeleteRoom(room.id)} className="flex-1 flex items-center justify-center gap-2 py-2 bg-red-500/10 text-red-400 rounded-lg hover:bg-red-500/20 border border-red-500/20 transition-all text-xs font-bold">
